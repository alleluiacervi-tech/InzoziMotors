...
--
-- This is src/schema.sql as it stood when migrations were introduced. Every
-- statement is idempotent (IF NOT EXISTS / ON CONFLICT DO NOTHING), which is
-- what makes it safe to record as "applied" against an existing database that
-- already has all of it — the runner executes it once, changes nothing, and
-- from here on the schema moves forward one numbered file at a time.
--
-- Do not edit this file. The runner checksums it and will refuse to start if it
