...
router.post('/:id/inquire', requireAuth, requireUuid('id'), async (req, res) => {
  ...
  if (startDate && !/^\d{4}-\d{2}-\d{2}$/.test(startDate)) return res.status(400).json({ error: 'start_date must be YYYY-MM-DD' });
  if (days !== null && (!Number.isInteger(days) || days < 1 || days > 365)) return res.status(400).json({ error: 'days must be between 1 and 365' });
    ...
    const inquiry = await withTransaction(async (client) => {
      const { rows } = await client.query(
        `SELECT rc.*, ${PROVIDER_COLUMNS},
         ...
         WHERE rc.id = $1 AND rc.status = 'active' FOR UPDATE OF rc`,
...
router.get('/:id', requireUuid('id'), async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT rc.*, ${PROVIDER_COLUMNS}
       FROM rental_cars rc LEFT JOIN users u ON u.id = rc.provider_id
       WHERE rc.id = $1 AND rc.status <> 'retired'`,
      [req.params.id]
    );
...
router.post('/', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    if (!provider.rowCount) return res.status(400).json({ error: 'Provider must be an active, identity- and business-verified seller' });
    const { rows } = await pool.query(
      `INSERT INTO rental_cars
       ...
       RETURNING *`,
      [provider_id, title, make, model, year, category, seats || 5, fuel,
       transmission, mileage, daily_rate, weekly_rate || Number(daily_rate) * 6,
       deposit || 0, min_days || 1, inspection_score, location, images || []]
    );
...
router.patch('/:id', requireAdmin, requireUuid('id'), async (req, res) => {
  ...
  try {
    if (req.body.provider_id) {
      ...
      if (!provider.rowCount) return res.status(400).json({ error: 'Provider must be active and verified' });
    }
