...
function NeighborhoodPin({ hood, carCount, isSelected, onPress, t }) {
  return (
    <Pressable
    ...
    >
      ...
      <View style={[styles.pinCount, isSelected && styles.pinCountSelected]}>
        <Text style={[styles.pinCountText, isSelected && styles.pinCountTextSelected]}>
          {t('map.carCount', { count: carCount, plural: carCount !== 1 ? 's' : '' })}
        </Text>
      </View>
    </Pressable>
  );
...
function CarListingCard({ car, onPress, t }) {
        ...
        <View style={styles.listingMeta}>
          {car.inspected && (
            <View style={styles.inspChip}>
              ...
              <Text style={styles.inspChipText}>{t('map.certified')}</Text>
            </View>
          )}
          <Text style={styles.listingDays}>{t('map.daysAgo', { count: daysAgo })}</Text>
        </View>
...
export default function MapScreen({ navigation }) {
  ...
  const categoryFilters = [
    ...
    { key: 'EV', label: t('home.evHybrid') },
    { key: 'Truck', label: t('home.truck') },
  ];
  ...
  return (
    <Screen background={colors.bg}>
      <BackHeader title={t('map.title')} onBack={() => navigation.goBack()} />
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View style={styles.mapBanner}>
          <View style={styles.mapBannerInner}>
            ...
            <View>
              <Text style={styles.mapBannerTitle}>{t('map.kigaliRwanda')}</Text>
              <Text style={styles.mapBannerSub}>{t('map.browseByNeighborhood')}</Text>
            </View>
          </View>
          <View style={styles.totalChip}>
            <Text style={styles.totalChipText}>{t('map.listingsCount', { count: cars.length })}</Text>
          </View>
        </View>
        ...
        <View style={styles.sectionHead}>
          <Text style={styles.sectionTitle}>{t('map.selectNeighborhood')}</Text>
        </View>
        ...
        <View style={styles.hoodResultHeader}>
          <View>
            ...
            <Text style={styles.hoodResultSub}>
              {activeHood?.area} · {activeHood?.description} · {t('map.carCount', { count: filteredCars.length, plural: filteredCars.length !== 1 ? 's' : '' })}
            </Text>
          </View>
          {filteredCars.length === 0 && categoryFilter !== 'All' && (
            <Pressable onPress={() => setCategoryFilter('All')}>
              <Text style={styles.clearFilter}>{t('map.clearFilter')}</Text>
            </Pressable>
          )}
        </View>
        ...
        {filteredCars.length === 0 ? (
          <View style={styles.emptyHood}>
            ...
            <Text style={styles.emptyHoodText}>
              {t('map.noListingsIn', { cat: categoryFilter !== 'All' ? categoryFilter : '', hood: activeHood?.name || '' })}
            </Text>
          </View>
        ) : (
          <FlatList
        ...
        >
          <Text style={styles.viewAllText}>{t('map.viewAllInKigali')}</Text>
